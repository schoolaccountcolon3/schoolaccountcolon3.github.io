...
class Networking {
    ...
    fetch = async (url, methods) => {
        ...
        if (url instanceof Request) {
            ...
            if (methods)
                requestObj = new Request(url, methods);
            else
            ...
            if (this.loopback.addressMap.has(port))
                ...
                if (anura.x86?.ready) {
                    return await new Promise(async (resolve) => {
                        let buffer = "";
                        let curlHeaders = "";
                        for (const header of requestObj.headers.entries()) {
                        ...
                        if (requestObj.body) {
                            ...
                            tmpFileName = "/tmp." + id;
                            await anura.fs.promises.writeFile(tmpFileName, (await requestObj.body?.getReader().read()).value);
                            curlHeaders += `--data "@/root${tmpFileName}" `;
                        }
                        const endMarker = crypto.randomUUID();
                        const pty = anura.x86.openpty(`/bin/ash -c 'curl -o - -s -i -X "${requestObj.method}" ${JSON.stringify(`http://localhost:${urlObj.port}${urlObj.pathname}${urlObj.search}`)} ${curlHeaders}| cat | base64 && echo -n "${endMarker}"'`, 0, 0, async (data) => {
                            try {
                                buffer += data;
                                if (buffer.endsWith(endMarker)) {
                                    ...
                                    const stringData = new TextDecoder("utf-8").decode(binaryData);
