...
class WispBus {
    ...
    static CLOSING = 2;
    static CLOSED = 3;
    setUpstreamProvider(upstreamProvider) {
        // TODO: close all client mappings assigned to upstream first
        this.upstream = upstreamProvider;
    }
    handleOutgoingPacket(packet, virtualServerID, packetCallBack) {
        ...
        if (parsed?.packetType == wisp.CONNECT) {
            ...
            for (const [id, serverMapping] of this.serverMappings.entries()) {
                if (serverMapping.regex.test(parsed.hostname)) {
                    ...
                    packetCallBack(wisp.createWispPacket({
            ...
            assignedHandler.send(wisp.createWispPacket({
        ...
        if (parsed?.packetType == wisp.DATA || parsed?.packetType == wisp.CLOSE) {
            const streamMetaData = this.clientMappings.get(this.virtualServerMapping
                .get(virtualServerID)
    ...
    handleIncomingPacket(packet, sourceID) {
        const parsed = wisp.parseIncomingPacket(packet);
    ...
    registerServer(regex, id, handler) {
        ...
        class FakeSocket extends EventTarget {
            ...
            constructor() {
                ...
                setTimeout(() => {
                    ...
                    this.dispatchEvent(new MessageEvent("message", { data: contPkt.buffer }));
                }, 0);
            }
            send(data) {
                ...
                bus.handleOutgoingPacket(pkt, this.virtualServerID, (response) => {
            ...
            close(code = 1000, reason = "") {
                if (this.readyState === WispBus.CLOSING ||
...
*/
//# sourceMappingURL=WispBus.js.map