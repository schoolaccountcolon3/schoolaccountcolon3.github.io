...
class LocalFS extends AFSProvider {
    ...
    name = "LocalFS";
    version = "1.0.0";
    path = Filer.Path; // replace with another polyfill
    stats = new Map();
    fds = [];
    cursors = [];
    ...
    async getChildDirHandle(path, recurseCounter = 0) {
    ...
    async getFileHandle(path, options, recurseCounter = 0) {
        if (!path.includes("/")) {
            path = "/" + path;
        }
        ...
        if (this.stats.has(realPath + "/" + fileName) &&
    ...
    static async newOPFS(anuraPath) {
        ...
        stat: async (path) => {
            ...
            try {
                ...
                try {
                    ...
                    let rootName;
                    if (!path)
                        rootName = this.domain.split("/").pop();
