...
class FilesAPI {
    ...
    async open(path) {
        ...
        if (extHandlers[ext]) {
            ...
            if (handler.handler_type === "cjs") {
                ...
                eval((await (await fetch(handler.path)).text()) +
                    `openFile(${JSON.stringify(path)})`); // here, JSON.stringify is used to properly escape the string
                return;
            }
        }
    ...
    async getIcon(path) {
        ...
        if (extHandlers[ext]) {
            ...
            if (handler.handler_type === "cjs") {
                // Legacy handler, eval it
                return eval((await (await fetch(handler.path)).text()) +
                    `if (getIcon) {
