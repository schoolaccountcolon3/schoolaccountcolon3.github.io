...
class Theme {
    ...
    static new(json) {
        return new Theme(json["foreground"], json["secondaryForeground"], json["border"], json["darkBorder"], json["background"], json["secondaryBackground"], json["darkBackground"], json["accent"]);
    }
    constructor(foreground = "#FFFFFF", secondaryForeground = "#C1C1C1", border = "#444444", darkBorder = "#000000", background = "#202124", secondaryBackground = "#383838", darkBackground = "#161616", accent = "#4285F4") {
        this.state = $state({
            foreground,
            secondaryForeground,
            border,
        ...
        for (const key in this.state) {
            useChange(use(this.state[key]), (value) => {
                for (const prop of this.cssPropMap[key]) {
    ...
    css() {
        ...
        for (const key in this.state) {
            for (const prop of this.cssPropMap[key]) {
                lines.push(`  ${prop}: ${this.state[key]};`);
            }
        }
        lines.push("}");
        return lines.join("\n");
