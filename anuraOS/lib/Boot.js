...
const clickoffChecker = (h("div", { class: [
        use(clickoffCheckerState.active, (active) => active
            ? css `
							position: absolute;
							width: 100%;
							height: 100%;
...
const updateClickoffChecker = (show) => {
...
window.addEventListener("load", async () => {
    ...
    if (await window.idbKeyval.get("bootFromOPFS")) {
        bootStrapFs = (await LocalFS.newSwOPFS());
    }
    try {
        ...
        conf = JSON.parse(new TextDecoder().decode(await bootStrapFs.promises.readFile("/config_cached.json")));
    }
    ...
    navigator.serviceWorker.addEventListener("message", (event) => {
        if (event.data.anura_target === "anura.sw.reinit")
            initComlink(); // this could accidentally be run twice but realistically there aren't any consequences for doing so
    });
    ...
    if (directories) {
        const needsMigration = Object.entries(map).filter(([key, [old, _new]]) => directories[key] === old);
        if (needsMigration.length > 0) {
            anura.notifications.add({
                title: "Anura Update",
                description: "AnuraOS has been updated to a new version. Users are recommended to change the installation directory of their apps and libraries to /usr/ to ensure consistency with new installations.",
                timeout: "never",
                buttons: [
                    {
                        text: "Migrate Now",
                        callback: async () => {
                            const migrate = async (oldPath, newPath) => {
                                const parent = newPath.split("/").slice(0, -1);
                                await sh.promises.mkdirp(parent.join("/"));
                                await anura.fs.promises.rename(oldPath, newPath);
                            };
                            await Promise.all(needsMigration.map(async ([key, [old, newPath]]) => {
    ...
    if (!requiredDirectories || !requiredDirectories.includes("bin")) {
        await anura.settings.set("requiredDirectories", (requiredDirectories = ["apps", "libs", "init", "bin", "opt"]));
    }
    requiredDirectories.forEach(async (k) => {
        if (!directories[k]) {
            directories[k] = defaultDirectories[k];
            await anura.settings.set("directories", directories);
        }
        try {
        ...
        try {
            ...
            if (files) {
                for (const file of files) {
                    ...
                    if (!file.endsWith(".init.ajs"))
                        continue;
                    const data = await anura.fs.promises.readFile(directories["init"] + "/" + file);
    ...
    launcher = new Launcher(clickoffChecker, updateClickoffChecker);
    quickSettings = new QuickSettings(clickoffChecker, updateClickoffChecker);
    calendar = new Calendar(clickoffChecker, updateClickoffChecker);
    taskbar = new Taskbar();
...
document.addEventListener("anura-login-completed", async () => {
    ...
    for (const bin of anura.config.bin) {
        ...
        try {
            ...
            await anura.fs.promises.writeFile(directories.bin + "/" + path, await fetch(bin).then((r) => r.text()));
        }
    }
    ...
    if (anura.platform.type === "mobile") {
        ...
        const tright = taskbar.element.querySelector("#taskbar-right");
        ...
        const aview = launcher.element.querySelector(".appsView");
    ...
    desktopCtx.addItem("Set wallpaper & style", () => {
...
async function bootUserCustomizations() {
    ...
    try {
        ...
        if (files) {
            for (const file of files) {
                const { type } = await anura.fs.promises.stat(`${directories["apps"]}/${file}`);
                if (type === "DIRECTORY") {
                    try {
                        await anura.registerExternalApp(`/fs/${directories["apps"]}/${file}/`);
                    }
                    catch (e) {
                    ...
                    const shortcut = JSON.parse((await anura.fs.promises.readFile(`${directories["apps"]}/${file}`)).toString());
