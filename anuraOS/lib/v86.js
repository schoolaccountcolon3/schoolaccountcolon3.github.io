...
V86.prototype.serial1_send = function (a) {
...
async function InitV86Hdd() {
    ...
    const size = (await new Promise((r) => (db.transaction("parts").objectStore("parts").get("size").onsuccess =
        r))).target.result;
    ...
    const fakefile = {
        ...
        slice: async (start, end) => {
            ...
            while (i <= endi) {
                ...
                if (!part) {
                    part = (await new Promise((r) => (db.transaction("parts").objectStore("parts").get(i).onsuccess =
                        r))).target.result;
        ...
        save: async (emulator = anura.x86?.emulator) => {
            ...
            for (const [offset, buffer] of emulator.disk_images.hda.block_cache) {
                ...
                if (!tmpb) {
                    const part = (await new Promise((r) => (db.transaction("parts").objectStore("parts").get(i).onsuccess =
                        r))).target.result;
...
class V86Backend {
    ...
    constructor(virt_hda) {
        ...
        this.emulator = new V86({
            ...
            cmdline: "rw init=/sbin/init root=/dev/sda rootfstype=ext4 random.trust_cpu=on 8250.nr_uarts=10 spectre_v2=off pti=off mitigations=off ipv6.disable=1",
    ...
    async onboot() {
        ...
        this.xpty = await this.openpty("/bin/ash -c 'startx /bin/xfrog'", 1, 1, async (data) => {
    ...
    openBinaryPty(command, cols, rows, onData) {
        if (!anura.x86.ready) {
    ...
    async startMouseDriver() {
        ...
        let pointer = "";
        const pty = await this.openpty("/bin/ash -c 'env DISPLAY=:0 /bin/anuramouse'", 100, 100, (data) => {
    ...
    async twispinit() {
        ...
        let recBuffer;
        const connections = { 0: { congestion: 4294967294 } };
        ...
        this.emulator.add_listener("virtio-console0-output-bytes", async (data) => {
            if (!remaniningLength) {
        ...
        let evalBuffer = "";
        this.emulator.add_listener("virtio-console1-output-bytes", async (data) => {
        ...
        function processIncomingWispFrame(frame) {
            ...
            switch (frame[0]) {
                ...
                case 3: // CONTINUE
                    if (connections[streamID]) {
                        console.log(`setting congestion for ${streamID} to ${view.getUint32(5, true)}`);
        ...
        //
        //
        this.sendWispFrame = async (frameObj) => {
            let fullPacket;
            let view;
