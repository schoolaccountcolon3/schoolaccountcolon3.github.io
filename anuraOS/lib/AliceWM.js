...
class WMWindow extends EventTarget {
    ...
    constructor(wininfo, app) {
        ...
        this.maximizeSvg = (h("svg", { xmlns: "http://www.w3.org/2000/svg", "xmlns:xlink": "http://www.w3.org/1999/xlink", width: "12px", height: "12px", viewBox: "0 0 12 12", version: "1.1" },
            h("g", { id: "surface1" },
                h("path", { style: " stroke:none;fill-rule:evenodd;fill-opacity:1;", d: "M 11.5 0.5 L 11.5 11.5 L 0.5 11.5 L 0.5 0.5 Z M 10 10 L 2 10 L 2 2 L 10 2 Z M 10 10 " }))));
        this.restoreSvg = (h("svg", { xmlns: "http://www.w3.org/2000/svg", "xmlns:xlink": "http://www.w3.org/1999/xlink", width: "12px", height: "12px", viewBox: "0 0 12 12", version: "1.1" },
            h("g", { id: "surface1" },
                h("path", { style: " stroke:none;fill-rule:evenodd;fill-opacity:1;", d: "M 11.5 8 L 4 8 L 4 0.5 L 11.5 0.5 Z M 5.5 6.5 L 5.5 2 L 10 2 L 10 6.5 Z M 2 4 L 0.5 4 L 0.5 11.5 L 8 11.5 L 8 10 L 2 10 Z M 2 4 " }))));
        this.element = (h("div", { class: "aliceWMwin opacity0", style: `
                    width: ${wininfo.width};
            ...
            (this.resizable && (h("div", { class: "resizers" },
                h("div", { class: "resize-edge left" }),
                h("div", { class: "resize-edge right" }),
                h("div", { class: "resize-edge top" }),
                h("div", { class: "resize-edge bottom" }),
                h("div", { class: "resize-corner top-left" }),
                h("div", { class: "resize-corner top-right" }),
                h("div", { class: "resize-corner bottom-left" }),
                h("div", { class: "resize-corner bottom-right" })))) ||
                "",
            h("div", { class: "title", "on:pointerdown": (evt) => {
                    deactivateFrames();
                ...
                h("div", { class: "titleContent" }, use(this.state.title)),
                h("button", { class: "windowButton minimize", "on:click": () => {
                        this.minimizing = true;
                        this.minimize();
                    } },
                    h("svg", { xmlns: "http://www.w3.org/2000/svg", "xmlns:xlink": "http://www.w3.org/1999/xlink", width: "12px", height: "12px", viewBox: "0 0 12 12", version: "1.1" },
                        h("g", { id: "surface1" },
                            h("path", { style: " stroke:none;fill-rule:evenodd;fill-opacity:1;", d: "M 0.5 10 L 11.5 10 L 11.5 11.5 L 0.5 11.5 Z M 0.5 10 " })))),
                h("button", { class: "windowButton maximize", "on:click": this.maximize.bind(this) }, (this.maximizeImg = this.maximizeSvg)),
                h("button", { class: "windowButton close", "on:click": this.close.bind(this) },
                    h("svg", { xmlns: "http://www.w3.org/2000/svg", "xmlns:xlink": "http://www.w3.org/1999/xlink", width: "12px", height: "12px", viewBox: "0 0 12 12", version: "1.1" },
                        h("g", { id: "surface1" },
                            h("path", { style: " stroke:none;fill-rule:evenodd;fill-opacity:1;", d: "M 11.5 1.609375 L 10.390625 0.5 L 6 4.890625 L 1.609375 0.5 L 0.5 1.609375 L 4.890625 6 L 0.5 10.390625 L 1.609375 11.5 L 6 7.109375 L 10.390625 11.5 L 11.5 10.390625 L 7.109375 6 Z M 11.5 1.609375 " }))))),
            (this.content = (h("div", { class: "content", style: "width: 100%; padding:0; margin:0;" })))));
        this.width = parseFloat(getComputedStyle(this.element, null)
            .getPropertyValue("width")
            .replace("px", ""));
        this.height = parseFloat(getComputedStyle(this.element, null)
        ...
        for (let i = 0; i < resizers.length; i++) {
            ...
            currentResizer.addEventListener("pointerdown", (e) => {
                e.preventDefault();
                if (anura.platform.type === "mobile" ||
                    anura.platform.type === "tablet")
                    return;
                original_width = parseFloat(getComputedStyle(this.element, null)
                    .getPropertyValue("width")
                    .replace("px", ""));
                original_height = parseFloat(getComputedStyle(this.element, null)
                    .getPropertyValue("height")
                    .replace("px", ""));
                deactivateFrames();
            ...
            const resize = (e) => {
                ...
                if (this.snapped) {
                    ...
                    const direction = this.getSnapDirectionFromPosition(original_x, original_width);
                    if ((direction === "left" &&
    ...
    handleDrag(evt) {
        ...
        if (this.clampWindows) {
            const newOffsetX = Math.min(window.innerWidth - this.element.clientWidth, Math.max(0, offsetX));
            const newOffsetY = Math.min(window.innerHeight - 49 - this.element.clientHeight, Math.max(0, offsetY));
            ...
            if (offsetX !== newOffsetX || offsetY !== newOffsetY) {
                ...
                const snapDirection = this.getSnapDirection(this.dragForceX, this.dragForceY);
                if (snapDirection) {
    ...
    async remaximize() {
        ...
        const animx = Math.abs(oldwidth - width) > 0.1 * Math.max(oldwidth, width) &&
            !anura.settings.get("disable-animation");
        const animy = Math.abs(oldheight - height) > 0.1 * Math.max(oldheight, height) &&
            !anura.settings.get("disable-animation");
    ...
    unminimize() {
        ...
        if (this.snapped) {
            const thisSnappedWindow = minimizedSnappedWindows.find((x) => x.window === this);
            ...
            minimizedSnappedWindows.splice(minimizedSnappedWindows.indexOf(thisSnappedWindow), 1);
            const leftSnappedWindows = snappedWindows.filter((x) => x.direction === "left");
            const rightSnappedWindows = snappedWindows.filter((x) => x.direction === "right");
            if (thisSnappedWindow.direction === "left") {
                const otherLeftSnappedWindows = leftSnappedWindows.filter((x) => x.window !== this);
            ...
            if (thisSnappedWindow.direction === "right") {
                const otherRightSnappedWindows = rightSnappedWindows.filter((x) => x.window !== this);
    ...
    snap(snapDirection) {
        ...
        if (snappedWindows.find((x) => x.direction === "left") &&
            snappedWindows.find((x) => x.direction === "right") &&
            !splitBar &&
            snapDirection !== "top" &&
            snappedWindows[0]?.direction !== snappedWindows[1]?.direction) {
            const bar = new WMSplitBar(snappedWindows.find((x) => x.direction === "left").window, snappedWindows.find((x) => x.direction === "right").window);
            bar.splitWindowsAround(width / 2);
            if (!anura.settings.get("disable-animation"))
                setTimeout(() => {
                    bar.leftWindow.element.classList.remove("remaxtransitionx");
        ...
        this.dispatchEvent(new MessageEvent("snap", {
    ...
    getSnapDirection(forceX, forceY) {
        if (forceX > 20 && forceY > 20) {
            ...
            if (this.element.offsetTop + this.element.clientHeight ==
    ...
    getSnapDirectionFromPosition(left, width) {
        if (left === 0) {
            return "left";
        }
    ...
    snapPreview(side) {
        ...
        const elem = (h("div", { class: `snapPreview-${side}`, id: "snapPreview", style: `${previewSide}: 0px; width: ${scaledWidth}px; height: ${scaledHeight}px; opacity: 0; ` }));
        setTimeout(() => {
...
class WMSplitBar {
    ...
    leftWindow;
    rightWindow;
    element = (h("div", { id: "snapSplitBar", style: `background-color: rgba(0, 0, 0, 0)`, "on:mouseover": () => {
            this.fadeIn();
        }, "on:mouseout": () => {
            this.fadeOut();
        }, "on:pointerdown": (evt) => {
...
function getHighestZindex() {
    const allWindows = Array.from(document.querySelectorAll(".aliceWMwin"));
    let highestZindex = 0;
...
//# sourceMappingURL=AliceWM.js.map