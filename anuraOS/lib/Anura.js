...
class Anura {
    ...
    static async new(config) {
        ...
        let fsProvider = new FilerAFSProvider(new Filer.FileSystem({
            name: "anura-mainContext",
            provider: new Filer.FileSystem.providers.IndexedDB(),
        }));
        if (await window.idbKeyval.get("bootFromOPFS")) {
            fsProvider = (await LocalFS.newSwOPFS());
        }
    ...
    async registerExternalApp(source) {
        try {
            ...
            if (shortcut.status === 200) {
                ...
                const app = new ShortcutApp(new URL(source, location.href).href, shortcutData);
