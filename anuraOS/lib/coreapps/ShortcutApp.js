...
function b26(s) {
    return [...s]
        .map((c) => [...c.charCodeAt(0).toString(26)]
...
class ShortcutApp extends App {
    static async launchShortcut(props) {
        ...
        const cmdline = (props.command.match(/(?:[^\s"]+|"[^"]*")+/g) || []).map((arg) => {
        ...
        const streams = anura.logger.createStreams("Shortcut: " + props.name + " (" + props.command + ") ");
        if (props.console) {
    ...
    console = false;
    command = '/usr/bin/vista.ajs --alert --message "Anura Shortcuts: This shortcut is not configured properly." --title Error';
    constructor(filePath, props) {
        ...
        if (anura.apps[this.package]) {
            ...
            console.warn("ShortcutApp: Mitigating package collision, please investigate as this is a bug.");
            anura.notifications.add({
                title: "ShortcutApp",
                description: "Package collision detected, renaming package, please investigate or report this.",
                timeout: 10000,
            });
        }
    }
...
//# sourceMappingURL=ShortcutApp.js.map