...
class TaskManager extends App {
    ...
    page = async () => (h("div", { style: {
        ...
        h("div", { class: "listwrapper" },
            h("table", { class: "list" },
                h("thead", null,
                    h("th", null, "Name"),
                    h("th", null, "Memory footprint"),
                    h("th", null, "Process ID")),
                h("tbody", null, use(anura.processes.state.procs, (procs) => procs.map((proc) => (h("tr", { class: "row", id: `taskmgr-proc-${proc.deref()?.pid}`, "class:selected": use(this.state.selected, (num) => num === proc.deref()?.pid), "on:click": () => {
                        if (proc.deref()) {
                    ...
                    h("td", null,
                        h("span", null,
                            ...
                            h("span", { class: "proc-title" }, proc.deref()?.title))),
                    h("td", null,
                        h("span", null, "Measuring")),
                    h("td", { class: "pid" },
                        h("span", { class: "pid" }, proc.deref()?.pid))))))))),
        h("div", { class: "controls" },
            h("button", { "bind:disabled": use(this.state.selected, (num) => num === -1), class: "matter-button-contained", "on:click": () => {
                    const proc = anura.processes.state.procs.find((proc) => proc && proc.deref()?.pid === this.state.selected);
                    if (proc) {
                        proc.deref()?.kill();
                    }
                } }, "End process"))));
    async open(args = []) {
        ...
        if (performance.measureUserAgentSpecificMemory) {
            const measure = async () => {
                ...
                const perfTree = await performance.measureUserAgentSpecificMemory();
                // type this maybe?
                const usage = {};
                for (const memUser of perfTree.breakdown) {
                    if (!memUser.attribution[0] ||
                        memUser.attribution[0].scope !== "Window" ||
                        !memUser.attribution[0].container)
                        continue;
                    if (!memUser.attribution[0].container.id.startsWith("proc-"))
                        continue;
                    const pid = Number(memUser.attribution[0].container.id.slice(5));
            ...
            while (page.parentElement) {
                ...
                const allRows = Array.from(document.querySelectorAll("[id^=taskmgr-proc-]").values());
