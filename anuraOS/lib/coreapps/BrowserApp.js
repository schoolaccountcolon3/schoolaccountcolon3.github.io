...
class BrowserApp extends App {
    ...
    constructor() {
        super();
        this.lib = new BrowserLib(this, (path, callback) => {
            ...
            if (typeof win === "undefined") {
                this.open().then((win) => {
                    ...
                    iframe?.addEventListener("load", () => {
                        ...
                        const browserDocument = iframe?.contentDocument || iframe?.contentWindow?.document;
                        const config = {
                            attributes: true,
                            subtree: true,
                        };
    ...
    async open(args = []) {
        if (args.length > 0) {
            ...
            const openTab = (path) => new Promise((resolve) => {
