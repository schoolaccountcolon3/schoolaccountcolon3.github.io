...
class Launcher {
    ...
    popupTransition = anura.settings.get("disable-animation")
        ? "opacity 0.15s"
        : "all 0.15s cubic-bezier(0.445, 0.05, 0.55, 0.95)";
    gridTransition = anura.settings.get("disable-animation")
        ? "all 0s"
    ...
    async init() {
        ...
        this.element = (h(Panel, { id: "launcher", width: anura.platform.type === "mobile" || anura.platform.type === "tablet"
                ? "100%"
                : "min(70%, 35em)", height: use(this.state.active, (active) => active
            ...
            h("div", { class: "topSearchBar" },
                h("img", { src: "/icon.png" }),
                h("input", { placeholder: "Search your tabs, files, apps, and more...", style: "outline: none; color: var(--theme-fg);", "bind:this": use(this.state.search), "on:input": this.handleSearch.bind(this) })),
            h("div", { id: "appsView", class: "appsView", "bind:this": use(this.state.appsView) }, use(this.state.apps, (apps) => (apps || []).map((app) => (h(LauncherShortcut, { app: app, onclick: () => {
                    this.hide();
                    app.open();
                } })))))));
    }
...
const LauncherShortcut = function () {
    ...
    contextmenu.addItem("Delete", async () => {
        if (anura.apps[app.package].source &&
            anura.apps[app.package].source.includes("/fs")) {
            try {
                const sh = new anura.fs.Shell();
                const path = app.source.replace(/^\/fs\//, "");
                await sh.rm(path, {
                    recursive: true,
                }, function (err) {
                    if (err)
                ...
                console.error(e);
                anura.dialog.alert("Could not delete app. Please try again later: " + e);
            }
        }
        else {
            console.error("App not found");
            anura.dialog.alert("App not found. Either it's a system app or something has gone terribly wrong.");
        }
    }, "delete");
    return (h("div", { class: "app", "on:click": this.onclick, "on:contextmenu": (e) => {
            e.preventDefault();
