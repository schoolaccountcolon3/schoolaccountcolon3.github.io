...
const SUPABASE_URL = "https://gpaddlvtftrspswmrnaz.supabase.co";
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImdwYWRkbHZ0ZnRyc3Bzd21ybmF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ4OTQ1MDEsImV4cCI6MjA5MDQ3MDUwMX0.tD2gVv6xfLeeqJU4AXNc8HCny18ya3FSWzbf9jjVS8Y";
...
try {
  ...
  async function populateUserList() {
    ...
    try {
      ...
      const currentSanitized = sanitizeFirebaseKey(userName);
      const userPromises = Object.keys(users).map(async (sanitizedName) => {
        ...
        const dmID = [currentSanitized, sanitizedName].sort().join('_');
        const dmSnap = await get(query(ref(database, `dms/${dmID}`), limitToLast(1)));
        let hasHistory = false;
        let hasUnread = false;

        if (dmSnap.exists()) {
          const msgs = Object.values(dmSnap.val());
          if (msgs.length > 0) {
            hasHistory = true;
            const lastMsgTime = new Date(msgs[0].timestamp).getTime();
            const lastOpenedTime = parseInt(localStorage.getItem(`chatLastOpened_${dmID}`) || '0', 10);
            if (lastMsgTime > lastOpenedTime && msgs[0].name !== userName) {
              hasUnread = true;
            }
          }
        }

        return { name: realName, pfp: userPfp, sanitized: sanitizedName, status: userStatus, hasHistory, hasUnread };
      });
      ...
      userList.forEach(userData => {
        ...
        if (userData.hasUnread) {
          userElement.classList.add('has-unread');
        } else if (userData.hasHistory) {
          userElement.classList.add('has-history');
        }
  ...
  function startDirectMessage(otherUserName) {
    ...
    localStorage.setItem(`chatLastOpened_${dmID}`, Date.now().toString());
    updateChatHeader();
    loadRecent();
    startLiveListener();
  }
  ...
  function showImageUploadModal() {
    return new Promise((resolve) => {
      ...
      async function handleFileChange() {
        ...
        try {
          const resizedData = await resizeImageForSending(file);
          imageData = { type: 'file', file: resizedData.blob, name: file.name };
          previewContainer.innerHTML = `<img src="${resizedData.preview}" alt="Preview"><p>Ready to upload via Supabase.</p>`;
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          if (img.width <= MAX_WIDTH) {
            return resolve({ blob: file, preview: e.target.result });
          }
          ...
          canvas.toBlob(blob => {
            resolve({ blob: blob, preview: canvas.toDataURL(file.type, 0.9) });
          }, file.type, 0.9);
        };
    ...
    imageUrlButton.addEventListener('click', async () => {
      ...
      if (result) {
        ...
        if (result.type === 'file') {
          ...
          const fileExt = result.name.split('.').pop() || 'png';
          ...
          try {
            const { error } = await supabase.storage.from(bucketName).upload(fileName, result.file, { upsert: false });
            if (error) throw error;
            ...
            showCustomAlert("Upload failed. Ensure Supabase 'chat-images' bucket is public and allows inserts.", "Upload Error");
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (inputHash === storedHash) {
          userName = enteredName; localStorage.setItem(localStorageNameKey, userName); localStorage.setItem(localStorageAuthKey, 'true'); isAuthenticated = true; userStatusRef = manageUserPresence(sanitizedName); showCustomAlert(`Login successful for ${userName}!`, 'Login Successful');
  ...
  const startLiveListener = () => {
    ...
    const unsubChildAdded = onChildAdded(liveQ, async snap => {
      ...
      if (currentChatContext.type === 'dm') {
        localStorage.setItem(`chatLastOpened_${currentChatContext.dmId}`, Date.now().toString());
        populateUserList(); // Refresh sidebar badges since we are marking as read
      } else if (currentChatContext.type === 'global' && msg.name !== userName) {
        // Note: If we implemented unread for global chat, it would go here
        populateUserList(); // To refresh unread badges if someone messages in DM while we are in global
      }
  ...
  async function initializeAppAsync() {
    ...
    if (storedUserName) {
      ...
      if (sessionInitiallyActive) {
        const sanitizedName = sanitizeFirebaseKey(storedUserName); const userCredRef = ref(database, `user_credentials/${sanitizedName}`);
        try {
          const snapshot = await get(userCredRef);
